#!/usr/bin/env python3
"""
Wikipedia JSON Enhancement System - Multi-API Key Version (Modified for New Structure)
Processes Wikipedia JSON files using multiple OpenAI API keys for parallel processing.
Reads from 'generated_section_content_md' and adds bullet points beneath it.
    ...
    def get_next_client(self) -> Tuple[OpenAI, int]:
        ...
        with self.lock:
            ...
            for i, key_name in enumerate(self.usage_stats.keys()):
                if self.usage_stats[key_name] == min_usage:
                    self.usage_stats[key_name] += 1
...
class WikipediaEnhancementManager:
    """
    Main manager class with multi-API key support for new JSON structure
    """
    
    def __init__(self, folder_path: str, model_name: str = "gpt-5", max_workers: int = None):
    ...
    def check_section_processed(self, section: Dict) -> bool:
        """Check if section has already been processed (has bullet points)"""
        return ('bullet_points' in section and 
                isinstance(section.get('bullet_points'), list) and 
                len(section.get('bullet_points', [])) > 0)
    ...
    def call_openai_api(self, content: str, chapter_name: str = "", section_name: str = "") -> Optional[List[str]]:
        ...
        user_prompt = f"""Your are an expert on indian foregin policy and Diplomacy. You task is to Create a detailed bullet point summary of the following content. THe content is from the book why bharat matter by s jaishankar.{context_info}
...
Return ONLY the bullet points, nothing else:"""
    ...
    def process_section_task(self, task: ProcessingTask) -> Tuple[bool, int, Dict]:
        ...
        # Create updated section data - keep all original fields and add bullet points
        updated_section = task.section.copy()
        updated_section['bullet_points'] = bullet_points
        updated_section['bullet_points_metadata'] = {
    ...
    def process_file_parallel(self, file_path: Path) -> bool:
        ...
        for i, section in enumerate(data):
            if not self.check_section_processed(section):
                ...
                # Read from generated_section_content_md field
                ...
                if content and len(content.strip()) >= 50:
                    ...
                    self.logger.warning(f"Skipping section {section_id}: no/insufficient content in generated_section_content_md")
    ...
    def print_final_report(self):
        ...
        for key_name, usage in usage_stats.items():
            console.print(f"  {key_name}: {usage} requests")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Multi-API Wikipedia JSON Enhancement System (Modified for New Structure)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python enhance_wikipedia_multi.py /path/to/json/folder
  python enhance_wikipedia_multi.py /path/to/json/folder --model gpt-5
...
Input JSON Structure:
  [
    {
      "chapter_name": "Organization Name",
      "chapter_id": "1",
      "section_number": "1",
      "section_name": "Introduction", 
      "generated_section_content_md": "Content to process..."
    }
  ]

Output adds 'bullet_points' field with summarized bullet points.
        """
    )
